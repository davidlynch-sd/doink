...
Editor * init_editor(const char *init_file_name) {
...
void render_editor(const Editor *editor) {
    ...
    mvprintw(LINES-3, 0,
             "DEBUG_INFO BUFFER_LEN: %zu BUFFER_USAGE: %zu BUFFER_INDEX: %zu CURSOR_X: %zu CURSOR_Y: %zu, LINES: %zu %s",
             editor->buffer_size,
             editor->buffer_usage,
             editor->buffer_index,
             editor->cursors[0].x,
...
void delete_char(Editor *editor) {
    ...
    editor->buffer[editor->buffer_index] = '\0';
    editor->buffer_usage--;
